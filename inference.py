    ...
    def initial_par(self):
        plans_file, fold, output_folder, dataset_directory, batch_dice, stage, unpack_data, deterministic, fp16 = \
            load_pickle(os.path.join(os.path.dirname(__file__), 'checkpoints/model_final_checkpoint.model.pkl')
                ...
                'init']
        plans = load_pickle(os.path.join(os.path.dirname(__file__), 'checkpoints/nnUNetPlansv2.1_plans_3D.pkl'))
...
class MuiltThreadDataGenerator(object):
    def __init__(self, iter, produce_queue_number) -> None:
        self.iter = iter
    ...
    def preprocess(self, nii_path):
        identity = nii_path.split('/')[-1].split('_0000.nii.gz')[0]
    ...
    def predict_model(self):
        ...
        while True:
            try:
                ...
                ori = ori.cuda().float()
                # predict
                predict = Valid_utils.predict_3D(ori, do_mirror=True, mirror_axes=(0, 1, 2))
                ...
                after_itk_label = sitk.GetImageFromArray(predict_final.astype(np.float32))
...
def inference():
    ...
    DG = MuiltThreadDataGenerator(file_list, 8)
...
if __name__ == '__main__':
    ...
    save_path = arg_s.input_folder
    base_folder_name = arg_s.output_folder
    # init model and predict utils
    patch_size = (96, 128, 160)
    model = my_Trainner().get_network(
        os.path.join(os.path.dirname(__file__), 'checkpoints', 'model_final_checkpoint.model'))
    model.cuda()
    model.eval()
    inference()
    pass
