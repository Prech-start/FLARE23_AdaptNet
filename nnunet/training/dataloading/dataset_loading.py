...
class DataLoader3D(SlimDataLoaderBase):
    ...
    # todo nnunet's determine_shapes
    def determine_shapes_nnunet(self):
    ...
    def determine_shapes(self):
        ...
        data_shape = (self.batch_size, num_color_channels, *self.patch_size)
        seg_shape = (self.batch_size, num_seg + 1, *self.patch_size)
        return data_shape, seg_shape

    # todo nnunet's generate batch
    def generate_train_batch_nnunet(self):
    ...
    def generate_train_batch(self):
        ...
        data = np.zeros(self.data_shape, dtype=np.float32)
        seg = np.zeros(self.seg_shape, dtype=np.float32)
        pseudo = np.zeros(self.seg_shape, dtype=np.float32)
        ret_weight = []
        case_properties = []
        for j, i in enumerate(selected_keys):
            # oversampling foreground will improve stability of model training, especially if many patches are empty
        ...
        #             if p == -1:
        #                 unique_true = np.delete(unique_true, 0)
        #                 unique_pseudo = np.delete(unique_pseudo, 0)
        ...
        #         mask = y_true > 0
        #         # filling seg
        #         seg[j, 0] = ~mask * y_pseudo + mask * y_true
        #         # weight calculation
        #         weight = [1 / 15] * 15
        #         before_weight = weight
        #         # todo adaptive scale is 0.5
        #         adapted_score = 1 / 15 * (len(unique_pseudo)) * (1 - 0.5)
        #         after_weight = [0.] * 15
        #         for i, w in enumerate(weight):
        #             if i in unique_pseudo and i is not 0:
        #                 after_weight[i] = 0.5 * before_weight[i]
        #             if i in unique_true:
        #                 after_weight[i] = before_weight[i] + adapted_score / (len(unique_pseudo) - 1 + 1e-6)
        #         after_weight = [w / (sum(after_weight) + 1e-6) for w in after_weight]
        #     else:
        #         after_weight = [1 / 15] * 15
        #     ret_weight.append(after_weight)
        # # print('data process time %s' % (time.time() - start))
        # case_properties.append(ret_weight)
        return {'data': data, 'seg': seg, 'properties': case_properties, 'keys': selected_keys}
